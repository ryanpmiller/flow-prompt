...
export const FormInput = forwardRef<HTMLInputElement, {
	...
	minLength?: number;
	error?: boolean;
} & Omit<React.InputHTMLAttributes<HTMLInputElement>, 'type' | 'placeholder' | 'value' | 'onChange' | 'required' | 'minLength'>>(({
	type = "text",
	placeholder,
	value,
	onChange,
...
export const FormTextarea = forwardRef<HTMLTextAreaElement, {
	...
	rows?: number;
	error?: boolean;
} & Omit<React.TextareaHTMLAttributes<HTMLTextAreaElement>, 'placeholder' | 'value' | 'onChange' | 'required' | 'rows'>>(({
	placeholder,
	value,
	onChange,
	required = false,
...
value: number;
	...
	max?: number;
	step?: number;
} & Omit<React.InputHTMLAttributes<HTMLInputElement>, 'type' | 'value' | 'onChange' | 'min' | 'max' | 'step'>) {
	return (
<input
			type="range"
			value={value}
...
onClick?: () => void;
	...
	icon?: ReactNode;
	loading?: boolean;
} & Omit<React.ButtonHTMLAttributes<HTMLButtonElement>, 'onClick' | 'disabled' | 'children' | 'type'>) {
	return (
<button
			type={type}
			onClick={onClick}
